...
<div id="tableSelectionModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p>Vui lòng chọn bàn phù hợp cho khách hàng:</p>
            <div class="table-selection">
                <c:forEach var="table" items="${listTable}">
                    <div class="table-option ${table.available ? '' : 'unavailable'}" 
        ...
        <div class="modal-footer">
            <button type="button" class="btn" style="background-color: #6c757d; color: white;" onclick="closeModal()">Hủy</button>
            <button type="button" class="btn btn-primary" id="confirmTableBtn" disabled>Xác nhận bàn</button>
        </div>
    </div>
</div>

<form id="confirmReservationForm" method="post" action="${pageContext.request.contextPath}/admin/Waiting_booking_table" style="display: none;">
    <input type="hidden" name="reservationId" id="selectedReservationId" />
...
<script>
    ...
    function fetchAvailableTables(guestCount) {
        fetch('${pageContext.request.contextPath}/admin/getAvailableTables?guestCount=' + guestCount)
            .then(response => response.json())
            .then(data => {
                populateTableOptions(data);
            })
            .catch(error => {
                console.error('Error fetching available tables:', error);
            
                simulateTableData(guestCount);
            });
    }
    
    function simulateTableData(guestCount) {
        const dummyTables = [];
        
        // Generate some dummy tables
        for (let i = 1; i <= 12; i++) {
            dummyTables.push({
                id: i,
                name: "Bàn " + i,
        ...
        populateTableOptions(dummyTables);
    }
    
    <%-- function populateTableOptions(tables) {
        const tableOptionsContainer = document.getElementById('tableOptions');
        List<Table> listTable = Tables.ro
        tableOptionsContainer.innerHTML = '';
        
        tables.forEach(table => {
            const isGoodFit = true; // Implement logic to determine if table is suitable for guest count
            
            const tableElement = document.createElement('div');
            tableElement.className = 'table-option' + (table.available ? '' : ' unavailable');
            ...
            tableOptionsContainer.appendChild(tableElement);
        });
    } --%>
    ...
    document.getElementById('confirmTableBtn').addEventListener('click', function() {
        if (selectedTableId) {
            document.getElementById('confirmReservationForm').submit();
        }
    });
